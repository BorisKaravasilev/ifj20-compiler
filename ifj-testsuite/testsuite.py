...
for test in tests:
    try:
        ...
        if process is not None:
            process.terminate()
        failed = failed + 1
        print(test['name'] + ': ' + tColors.RED + 'FAILED' + tColors.ENDC)
    else:
        if result:
            passed = passed + 1
            print(test['name'] + ': ' + tColors.GREEN + 'PASSED' + tColors.ENDC)
        else:
            skiped = skiped + 1
            print(test['name'] + ': ' + tColors.YELLOW + 'SKIPED' + tColors.ENDC)
if log is not None:
    log.close()
if os.path.isdir(args.tmp_dir):
...
print('\n-------- SUMMARY --------\n')
print('PASSED: ' + tColors.GREEN + str(passed) + tColors.ENDC)
print('FAILED: ' + tColors.RED + str(failed) + tColors.ENDC)
print('SKIPED: ' + tColors.YELLOW + str(skiped) + tColors.ENDC)
